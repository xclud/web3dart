...
/// This extension provides Dart methods around the raw [OkxWallet] JavaScript
/// object.
///
...
/// [asRpcService].
extension DartOkxWallet on OkxWallet {
  ...
  Future<dynamic> rawRequest(String method, {Object? params}) {
    ...
    final args = params == null ? RequestArguments(method: method) : RequestArguments(method: method, params: params);
...
class _OkxWalletRpcService extends RpcService {
  final OkxWallet _okxWallet;

  _OkxWalletRpcService(this._okxWallet);

  @override
  Future<RPCResponse> call(String function, [List? params]) {
    return _okxWallet.rawRequest(function, params: params).then((res) {
...
class _EventStreamSubscription extends StreamSubscription<dynamic> {
  ...
  _EventStreamSubscription(this._client, this._eventName, Function(dynamic)? onData) {
    if (_onData != null) {
  ...
  void onData(void Function(dynamic data)? handleData) {
    ...
    _stopListening();
    _onData = handleData == null ? null : Zone.current.bindUnaryCallback(handleData);
    _resumeIfNecessary();
  }
